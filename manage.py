...
def parse_args():
    parser = argparse.ArgumentParser(
        description="PortHound standalone launcher."
    )
    parser.add_argument(
        "mode",
        nargs="?",
    ...
    parser.add_argument("--role", choices=("standalone",), default="standalone")
    ...
    parser.add_argument("--master-host", help=argparse.SUPPRESS)
    parser.add_argument("--master-ip", help=argparse.SUPPRESS)
    parser.add_argument("--ip", help=argparse.SUPPRESS)
    parser.add_argument("--ca", help=argparse.SUPPRESS)
    parser.add_argument("--ca-oneline", help=argparse.SUPPRESS)
    ...
    parser.add_argument("--agent-cert", help=argparse.SUPPRESS)
    parser.add_argument("--agent-key", help=argparse.SUPPRESS)
    parser.add_argument("--agent-id", help=argparse.SUPPRESS)
    parser.add_argument("--agent-token", help=argparse.SUPPRESS)
    parser.add_argument("--agent-shared-key", help=argparse.SUPPRESS)
    parser.add_argument("--agent-poll-seconds", type=int, help=argparse.SUPPRESS)
    parser.add_argument("--agent-http-timeout", type=float, help=argparse.SUPPRESS)
    parser.add_argument("--agent-task-lease-seconds", type=int, help=argparse.SUPPRESS)
    parser.add_argument("--agent-tls-check-hostname", choices=BOOL_CHOICES, help=argparse.SUPPRESS)
...
def main():
    ...
    has_legacy_mode_inputs = any(
        [
            str(getattr(args, "mode", "") or "").strip(),
            str(getattr(args, "enroll_payload", "") or "").strip(),
            str(getattr(args, "agent_enroll", "") or "").strip(),
        ]
    )
    if has_legacy_mode_inputs:
        print(
            "[bootstrap] agent/master enrollment inputs are deprecated and ignored. "
            "PortHound now runs as standalone only."
        )

    args.role = "standalone"
    args.host = str(FIXED_WEB_HOST)
    if int(getattr(args, "port", 0) or 0) not in {0, int(FIXED_WEB_MASTER_PORT)}:
        print(
            "[bootstrap] fixed standalone port policy active: "
            f"using {FIXED_WEB_MASTER_PORT}."
        )
    args.port = int(FIXED_WEB_MASTER_PORT)

    if (
        str(getattr(args, "db_path", "") or "").strip() == ""
        and str(environ.get("PORTHOUND_DB_PATH", "") or "").strip() == ""
    ):
        args.db_path = default_db_path_for_role("standalone")

    load_env_fallbacks(args)
    apply_cli_overrides(args)

    environ["PORTHOUND_ROLE"] = "standalone"
    ...
    final_db_path = (
        ...
        or str(getattr(args, "db_path", "") or "").strip()
        or default_db_path_for_role("standalone")
    )
    environ["PORTHOUND_DB_PATH"] = final_db_path
