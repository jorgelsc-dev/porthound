# GitHub Publication Kit - PortHound

## 1) Repo identity
- Name: `PortHound`
- Suggested URL: `https://github.com/<tu-usuario>/porthound`
- Visibility: `Public`
- License: `MIT`
...
### Website
`https://porthound.jorgelsc.dev`

### Apply from terminal (GitHub CLI)

```bash
gh auth login
gh repo edit jorgelsc-dev/porthound \
...
PortHound is a Python-based distributed network scanner with resumable TCP/UDP scans, banner fingerprinting, and a lightweight HTTP/WS control plane.
...
git init
git add .
git commit -m "chore: initial public release of PortHound"
git branch -M main
git remote add origin https://github.com/<tu-usuario>/porthound.git
...
```bash
git remote set-url origin git@github.com:<tu-usuario>/porthound.git
git push -u origin main
```
...
6. Enable GitHub Pages and point the custom domain to `porthound.jorgelsc.dev`.
